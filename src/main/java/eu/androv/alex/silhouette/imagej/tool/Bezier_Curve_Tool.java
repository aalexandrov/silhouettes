package eu.androv.alex.silhouette.imagej.tool;
...
//				imp.setRoi(roi);
	...
	private void addCurve(Overlay overlay) {
		...
		curveROI.setStrokeColor(Color.magenta);
	...
	class BezierList {
		...
		public GeneralPath getCurve() {
			if (bezierStart==null) return null;
	...
	class Bezier {
		...
		public void insertAsNext(Bezier newBezier) {
			...
			if (next != null) next.setPrevious(newBezier);
			next = newBezier;
		...
		public void insertAsPrevious(Bezier newBezier) {
			...
			if (previous != null) previous.setNext(newBezier);
			previous = newBezier;
